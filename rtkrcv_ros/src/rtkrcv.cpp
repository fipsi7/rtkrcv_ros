 ...
 *                           change file paths of solution status and debug
 *trace 2015/01/10 1.11 add line editting and command history separate codes for
 *virtual console to vt.c
 *-----------------------------------------------------------------------------*/
#include <signal.h>
extern "C" {
#include "rtklib.h"
...
static int soltype =
    0; /* sol format (0:dms,1:deg,2:xyz,3:enu,4:pyl,5:vel_enu) */
...
static int strfmt[] = {                /* stream formats */
                       STRFMT_UBX, STRFMT_RTCM3, STRFMT_SP3, SOLF_LLH,
                       SOLF_NMEA};
...
static const char *pathopts[] =
    {/* path options help */
     "stream path formats",
     " serial   : "
     "port[:bit_rate[:byte[:parity(n|o|e)[:stopb[:fctr(off|on)]]]]]",
...
#define FMTOPT                                                                 \
  "0:rtcm2,1:rtcm3,2:oem4,3:oem3,4:ubx,5:ss2,6:hemis,7:skytraq,8:gw10,9:"      \
  "javad,10:nvs,11:binex,12:rt17,15:sp3"
#define NMEOPT "0:off,1:latlon,2:single"
#define SOLOPT "0:llh,1:xyz,2:enu,3:nmea,4:gisf,5:vel_enu,6:nmea_psv"
...
static int startsvr(vt_t *vt) {
  ...
  if (!rtksvrstart(&svr, svrcycle, buffsize, strtype, paths, strfmt, navmsgsel,
                   cmds, ropts, nmeacycle, nmeareq, npos, &prcopt, solopt,
                   &moni)) {
...
static void prtime(vt_t *vt, gtime_t time) {
  ...
  if (timetype == 1) {
    ...
    sprintf(tstr, "  %04d %8.1f", week, tow);
  } else
...
static void prsolution(vt_t *vt, const sol_t *sol, const double *rb) {
  const char *solstr[] = {"------", "FIX",    "FLOAT", "SBAS",
                          "DGPS",   "SINGLE", "PPP",   ""};
  double pos[3] = {0}, Qr[9], Qe[9] = {0}, dms1[3] = {0}, dms2[3] = {0},
         bl[3] = {0};
  ...
  if (soltype == 0) {
    ...
    vt_printf(vt, " %s:%2.0f %02.0f %07.4f", pos[0] < 0 ? "S" : "N",
              fabs(dms1[0]), dms1[1], dms1[2]);
    vt_printf(vt, " %s:%3.0f %02.0f %07.4f", pos[1] < 0 ? "W" : "E",
              fabs(dms2[0]), dms2[1], dms2[2]);
    vt_printf(vt, " H:%8.3f", pos[2]);
    if (solflag & 1) {
      vt_printf(vt, " (N:%6.3f E:%6.3f U:%6.3f)", SQRT(Qe[4]), SQRT(Qe[0]),
                SQRT(Qe[8]));
    }
  } else if (soltype == 1) {
    if (norm(sol->rr, 3) > 0.0) {
    ...
    vt_printf(vt, " H:%8.3f", pos[2]);
    if (solflag & 1) {
      vt_printf(vt, " (E:%6.3f N:%6.3f U:%6.3fm)", SQRT(Qe[0]), SQRT(Qe[4]),
                SQRT(Qe[8]));
    }
  } else if (soltype == 2) {
    ...
    if (solflag & 1) {
      vt_printf(vt, " (X:%6.3f Y:%6.3f Z:%6.3f)", SQRT(Qr[0]), SQRT(Qr[4]),
                SQRT(Qr[8]));
    }
  } else if (soltype == 3) {
    if (len > 0.0) {
    ...
    if (solflag & 1) {
      vt_printf(vt, " (E:%6.3f N:%6.3f U:%6.3f)", SQRT(Qe[0]), SQRT(Qe[4]),
                SQRT(Qe[8]));
    }
  } else if (soltype == 4) {
    if (len > 0.0) {
...
static void prstatus(vt_t *vt) {
  ...
  const char *sol[] = {"-",    "fix",    "float", "SBAS",
                       "DGPS", "single", "PPP",   ""};
  const char *mode[] = {"single",      "DGPS",  "kinematic",  "static",
  ...
  for (i = 0; i < 3; i++) {
    ...
    vt_printf(vt,
              "%-28s: "
              "obs(%d),nav(%d),gnav(%d),ion(%d),sbs(%d),pos(%d),dgps(%d),ssr(%"
              "d),err(%d)\n",
  ...
  vt_printf(vt, "%-28s: %s\n", "time of receiver clock rover",
            rtk.sol.time.time ? tstr : "-");
  vt_printf(vt, "%-28s: %.9f\n", "time sys offset (glo-gps)(s)",
            rtk.sol.dtr[1]);
...
static void prsatellite(vt_t *vt, int nf) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    vt_printf(vt, " ");
    for (j = 0; j < nf; j++)
      vt_printf(vt, "%6.3f", rtk.ssat[i].resp[j]);
    for (j = 0; j < nf; j++)
      vt_printf(vt, "%7.4f", rtk.ssat[i].resc[j]);
...
static void probserv(vt_t *vt, int nf) {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < nf; j++)
      vt_printf(vt, "%12.2f", obs[i].P[j]);
    for (j = 0; j < nf; j++)
      vt_printf(vt, "%13.2f", obs[i].L[j]);
    for (j = 0; j < nf; j++)
...
static void prnavidata(vt_t *vt) {
  ...
  vt_printf(vt, "\n%s%3s %3s %3s %3s %3s %3s %3s %19s %19s %19s %3s %3s%s\n",
            ESC_BOLD, "SAT", "S", "IOD", "IOC", "FRQ", "A/A", "SVH", "Toe",
            "Toc", "Ttr/Tof", "L2C", "L2P", ESC_RESET);
  for (i = 0; i < MAXSAT; i++) {
    ...
    vt_printf(vt, "%3s %3s %3d %3d %3d %3d  %02X %19s %19s %19s %3d %3d\n", id,
              valid ? "OK" : "-", eph[i].iode, eph[i].iodc, 0, eph[i].sva,
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    if (geph[prn - 1].tof.time != 0)
      ...
      strcpy(s2, "-");
    vt_printf(vt, "%3s %3s %3d %3d %3d %3d  %02X %19s %19s %19s %3d %3d\n", id,
              valid ? "OK" : "-", geph[prn - 1].iode, 0, geph[prn - 1].frq,
...
static void prstream(vt_t *vt) {
  const char *ch[] = {"input rover", "input base",  "input corr",
                      "output sol1", "output sol2", "log rover",
                      "log base",    "log corr",    "monitor"};
  const char *type[] = {"-",      "serial", "file", "tcpsvr", "tcpcli", "udp",
                        "ntrips", "ntripc", "ftp",  "http",   "ros"};
  const char *fmt[] = {"rtcm2", "rtcm3", "oem4",    "oem3", "ubx",
                       "ss2",   "hemis", "skytreq", "gw10", "javad",
                       "nvs",   "binex", "rt17",    "",     "",
                       "sp3",   "",      "",        ""};
  ...
  vt_printf(vt, "\n%s%-12s %-6s %-5s %s %9s %7s %9s %7s %s%s\n", ESC_BOLD,
            "Stream", "Type", "Fmt", "S", "In-byte", "In-bps", "Out-byte",
            "Out-bps", "Message", ESC_RESET);
  for (i = 0; i < 9; i++) {
...
static void cmd_save(char **args, int narg, vt_t *vt) {
  ...
  if (!saveopts(file, "w", comment, rcvopts) ||
      !saveopts(file, "a", NULL, sysopts)) {
 ...
 * -------------------------------------------------------*/
bool cmd_rcv(rtkrcv_ros::receive_command::Request &cmd,
             rtkrcv_ros::receive_command::Response &res) {

  const char *cmds[] = {"start", "stop", "restart", "load", ""};
  int i, j, narg;
  char *args[MAXARG], *p;
  ...
  if (narg > 0) {
    ...
    switch (j) {
    case 0:
      res.success = srv_cmd_start(args, narg, vt_ptr, &ack);
      break;
    case 1:
      res.success = srv_cmd_stop(args, narg, vt_ptr, &ack);
      break;
    case 2:
      res.success = srv_cmd_restart(args, narg, vt_ptr, &ack);
      break;
    case 3:
      res.success = srv_cmd_load(args, narg, vt_ptr, &ack);
      break;
    }
    res.ack_str = ack;
  }
...
void *srv_thread(void *arg) {
  ...
  ros::ServiceServer service =
      node_handle.advertiseService(strcat(topic_name, "_cmd"), cmd_rcv);
...
static void cmdshell(vt_t *vt) {
  const char *cmds[] = {"start",     "stop",   "restart",  "solution", "status",
                        "satellite", "observ", "navidata", "stream",   "error",
                        "option",    "set",    "load",     "save",     "log",
                        "help",      "?",      "exit",     "shutdown", ""};
  int i, j, narg;
  ...
  while (!intflg) {
    ...
    /* input command */
    if (!vt_gets(vt, buff, sizeof(buff)))
      break;
    ...
    switch (j) {
    case 0:
      cmd_start(args, narg, vt);
      break;
    case 1:
      cmd_stop(args, narg, vt);
      break;
    case 2:
      cmd_restart(args, narg, vt);
      break;
    case 3:
      cmd_solution(args, narg, vt);
      break;
    case 4:
      cmd_status(args, narg, vt);
      break;
    case 5:
      cmd_satellite(args, narg, vt);
      break;
    case 6:
      cmd_observ(args, narg, vt);
      break;
    case 7:
      cmd_navidata(args, narg, vt);
      break;
    case 8:
      cmd_stream(args, narg, vt);
      break;
    case 9:
      cmd_error(args, narg, vt);
      break;
    case 10:
      cmd_option(args, narg, vt);
      break;
    case 11:
      cmd_set(args, narg, vt);
      break;
    case 12:
      cmd_load(args, narg, vt);
      break;
    case 13:
      cmd_save(args, narg, vt);
      break;
    case 14:
      cmd_log(args, narg, vt);
      break;
    case 15:
      cmd_help(args, narg, vt);
      break;
    case 16:
      cmd_help(args, narg, vt);
      break;
 ...
 *
 * description
 *     A command line version of the real-time positioning AP by rtklib. To
 *start or stop RTK server, to configure options or to print solution/status,
 *     login a console and input commands. As default, stdin/stdout are used for
 *     the console. Use -p option for network login with telnet protocol. To
 *show the available commands, type ? or help on the console. The initial
 *     processing options are loaded from default file rtkrcv.conf. To change
 *the file, use -o option. To configure the processing options, edit the options
