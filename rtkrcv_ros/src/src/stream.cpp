...
/* open ros publisher
 * -----------------------------------------------------------*/
static ros_pub_t *openRosPub(const char *path, int out_format, int time_format,
                             char *msg) {
  ...
  if (out_format == 0 || out_format == 1 || out_format == 2 ||
    ...
    return ros_pub;
  } else if (out_format == 6) {
    ros_pub->publisher = node->advertise<nmea_msgs::Sentence>(path, 10);
    return ros_pub;
  }
}
...
static int publish_ros_msg(ros_pub_t *ros_pub, unsigned char *buff, int n) {
  ...
  if (n > 0) {
    ...
    if (ros_pub->output_format == 0 || ros_pub->output_format == 1 ||
        ros_pub->output_format == 2) {
      ...
      if (ros_pub->time_format == 1) {
        ...
        sscanf((char *)buff,
               "%lu/%lu/"
               "%lu\t%lu:%lu:%lf\t%lf\t%lf\t%lf\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%"
               "lf\t%lf\t%lf\t%lf",
               &msg.year, &msg.month, &msg.day, &msg.hour, &msg.min, &msg.sec,
               &msg.data[0], &msg.data[1], &msg.data[2], &msg.Q, &msg.ns,
               &msg.std[0], &msg.std[1], &msg.std[2], &msg.std[3], &msg.std[4],
               &msg.std[5], &msg.age, &msg.ratio);
        ...
        msg.header.stamp = ros::Time::now();
        ros_pub->output_format == 2
            ? msg.header.frame_id = std::string("local")
            : msg.header.frame_id = std::string("world");
        sscanf((char *)buff,
               "%lu\t%lf\t%lf\t%lf\t%lf\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%"
               "lf\t%lf\t%lf",
               &msg.week, &msg.tow, &msg.data[0], &msg.data[1], &msg.data[2],
               &msg.Q, &msg.ns, &msg.std[0], &msg.std[1], &msg.std[2],
               &msg.std[3], &msg.std[4], &msg.std[5], &msg.age, &msg.ratio);
        ros_pub->publisher.publish(msg);
      }
      ...
      while (ptr <= (char *)buff + n - 2) {
        ...
        if (single_msg[3] == 'R' && single_msg[4] == 'M' &&
            single_msg[5] == 'C') { // GPRMC
          sscanf((char *)buff,
                 "$GPRMC,%f,%c,%lf,%c,%lf,%c,%f,%f,%d,%f,%c,%c*%s\n", &rmc.time,
                 &rmc.validity, &rmc.latitude, &rmc.char_lat, &rmc.longitude,
                 &rmc.char_lon, &rmc.speed, &rmc.course, &rmc.date,
                 &rmc.variation, &rmc.east_west, &rmc.mode, &rmc.checksum);
          msg.gprmc.hour = (int)rmc.time / 10000;
          msg.gprmc.min = (int)rmc.time / 100 - msg.gprmc.hour * 100;
          msg.gprmc.sec =
              rmc.time - msg.gprmc.min * 100 - msg.gprmc.hour * 10000;
          rmc.validity == 'A' ? msg.gprmc.validity = msg.gprmc.A
                              : msg.gprmc.validity = msg.gprmc.V;
          msg.gprmc.lat.deg = (int)rmc.latitude / 100;
          msg.gprmc.lat.min = (int)rmc.latitude - msg.gprmc.lat.deg * 100;
          msg.gprmc.lat.sec =
              (rmc.latitude - msg.gprmc.lat.deg * 100 - msg.gprmc.lat.min) *
              60.0;
          rmc.char_lat == 'N' ? msg.gprmc.lat.direction = msg.gprmc.lat.NORTH
                              : msg.gprmc.lat.direction = msg.gprmc.lat.SOUTH;
          msg.gprmc.lat.coord_deg =
              (msg.gprmc.lat.deg + msg.gprmc.lat.min / 60.0 +
               msg.gprmc.lat.sec / 60.0 / 60.0);
          ...
          msg.gprmc.lon.sec =
              (rmc.longitude - msg.gprmc.lon.deg * 100 - msg.gprmc.lon.min) *
              60.0;
          rmc.char_lon == 'W' ? msg.gprmc.lon.direction = msg.gprmc.lon.WEST
                              : msg.gprmc.lon.direction = msg.gprmc.lon.EAST;
          msg.gprmc.lon.coord_deg =
              (msg.gprmc.lon.deg + msg.gprmc.lon.min / 60.0 +
               msg.gprmc.lon.sec / 60.0 / 60.0);
          ...
          msg.gprmc.year =
              rmc.date - msg.gprmc.month * 100 - msg.gprmc.day * 10000;
        ...
        } else if (single_msg[3] == 'G' && single_msg[4] == 'G' &&
                   single_msg[5] == 'A') { // GPGGA
          sscanf((char *)single_msg,
                 "$GPGGA,%f,%lf,%c,%lf,%c,%d,%d,%lf,%lf,%c,%lf,%c,%lf,%s*%s\n",
                 &gga.time, &gga.latitude, &gga.char_lat, &gga.longitude,
                 &gga.char_lon, &gga.quality, &gga.nsat, &gga.hdop,
                 &gga.altitude, &gga.alt_units, &gga.undulation, &gga.u_units,
                 &gga.age, &gga.station_id, &gga.checksum);
          msg.gpgga.hour = (int)gga.time / 10000;
          msg.gpgga.min = (int)gga.time / 100 - msg.gpgga.hour * 100;
          msg.gpgga.sec =
              gga.time - msg.gpgga.min * 100 - msg.gpgga.hour * 10000;
          msg.gpgga.lat.deg = (int)gga.latitude / 100;
          msg.gpgga.lat.min = (int)gga.latitude - msg.gpgga.lat.deg * 100;
          msg.gpgga.lat.sec =
              (gga.latitude - msg.gpgga.lat.deg * 100 - msg.gpgga.lat.min) *
              60.0;
          gga.char_lat == 'N' ? msg.gpgga.lat.direction = msg.gpgga.lat.NORTH
                              : msg.gpgga.lat.direction = msg.gpgga.lat.SOUTH;

          msg.gpgga.lat.coord_deg =
              (msg.gpgga.lat.deg + msg.gpgga.lat.min / 60.0 +
               msg.gpgga.lat.sec / 60.0 / 60.0);
          ...
          msg.gpgga.lon.sec =
              (gga.longitude - msg.gpgga.lon.deg * 100 - msg.gpgga.lon.min) *
              60.0;
          gga.char_lon == 'W' ? msg.gpgga.lon.direction = msg.gpgga.lon.WEST
                              : msg.gpgga.lon.direction = msg.gpgga.lon.EAST;
          msg.gpgga.lon.coord_deg =
              (msg.gpgga.lon.deg + msg.gpgga.lon.min / 60.0 +
               msg.gpgga.lon.sec / 60.0 / 60.0);
        ...
        } else if (single_msg[3] == 'G' && single_msg[4] == 'S' &&
                   single_msg[5] == 'A') { // G*GSA
          ...
          if (*ptr1 != ',') {
            ...
            for (int j = 0; j < 2;
                 j++) { // discard #messages and message_sequence fields
              while (*ptr2 != ',')
                ptr2++;
              ptr1 = ++ptr2;
            }
      ...
      if (ros_pub->time_format == 1) {
        ...
        ros_pub->output_format == 2
            ? msg.header.frame_id = std::string("local")
            : msg.header.frame_id = std::string("VEL_ENU");
        sscanf((char *)buff,
               "%lu/%lu/"
               "%lu\t%lu:%lu:%lf\t%lf\t%lf\t%lf\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%"
               "lf\t%lf\t%lf\t%lf",
    ...
    } else if (ros_pub->output_format == 6) {
      /*NMEA*/
      ...
      while (ptr <= (char *)buff + n - 2) {
        ...
        msg.sentence = std::string(single_msg);
        ros_pub->publisher.publish(msg);
...
extern int stropen_out_sol(stream_t *stream, int type, int mode, char *path,
                           int out_format, int time_format) {
...
extern int strwrite(stream_t *stream, unsigned char *buff, int n) {
  ...
  case STR_ROS_MSG:
    /*Write mode: create a ros publisher*/
    ns = publish_ros_msg((ros_pub_t *)stream->port, buff, n);
    break;
  case STR_FTP:
  case STR_HTTP:
  default:
  ...
  if ((int)(tick - stream->tick) > tirate) {
    stream->outr =
        (stream->outb - stream->outbt) * 8000 / (tick - stream->tick);
    stream->tick = tick;
